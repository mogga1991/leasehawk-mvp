#!/usr/bin/env python3
"""
Script to create tables and seed Neon database with sample GSA prospectus data
"""
import sys
import os
...
def seed_database():
    ...
    try:
        ...
        existing_prospectuses = db.query(Prospectus).count()
        if existing_prospectuses > 0:
            print(f"📊 Database already contains {existing_prospectuses} prospectuses")
        ...
        for i, prospectus in enumerate(prospectuses):
            if i < len(properties):
                ...
                if i + 1 < len(properties):
                    match2 = Match(
                        prospectus_id=prospectus.id,
                        property_id=properties[(i + 1) % len(properties)].id,
                        total_score=0.75,
                        size_score=0.80,
                        location_score=0.85,
                        parking_score=0.70,
        ...
        print(f"  • ${sum(p.estimated_annual_cost for p in prospectuses if p.estimated_annual_cost):,.0f} total pipeline value")
